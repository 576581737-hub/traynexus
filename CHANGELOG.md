...
## v1.0722.2 - 2026-07-25（自动亮度接管 + 平滑 + 效率）

### 自动亮度重构（单一控制源）
- **接管系统亮度**：开启自动亮度时自动关闭 **Windows 自适应亮度**（`powercfg` VIDEOADAPT），检测 AMD 显卡并尝试关闭 **Vari-Bright**（驱动级自适应亮度），消除两个控制源抢同一亮度通道导致的跳变/抖动
  - 这正是"微信截图后屏幕变暗不恢复"类 bug 的根因之一（Vari-Bright 对暗遮罩产生负反馈、退出后不恢复）
  - 关闭自动亮度 / 退出程序时**恢复**系统原始设置（受 `AutoBrightnessRestoreOnExit` 开关控制，默认开）
  - 无管理员权限时无法接管，会托盘提示"需以管理员身份运行"
- **平滑调节**：lux→亮度改为连续分段曲线（锚点插值），配合 **EMA 指数滑动平均** 与 **3% 亮度死区迟滞**，告别旧版 5 档硬跳变与边界抖动
- **尊重手动调节**：用户用 Fn 键手动调亮度后，自动亮度**暂停 45 秒**再接管，不再立刻覆盖用户设定
- **效率**：光传感器读取从"每次新开 `powershell.exe` 进程"改为**复用常驻 PowerShell Runspace**，采样间隔 10s → 15s

### 其他
- 新增 `AdaptiveBrightnessController.cs`
- 版本号统一 1.0722.2（代码 / 安装脚本）

---

## v1.0722.1 - 2026-07-22（自动亮度 + 更新检查 + 诊断页修复）

### 新功能
- **自动亮度**：通过 WinRT LightSensor API 读取环境光照度，按光照梯度映射到亮度百分比
  - 无传感器时开关灰色不可用（"未检测到环境光传感器"）
  - 硬件存在但数据不可用时显示"硬件已检测，数据不可用"
- **更新检查**：在设置页"关于"区域新增检查更新按钮
  - 启动后延时检查，有新版弹托盘气泡通知
  - 手动检查弹 MessageBox 确认后跳转 GitHub Releases
  - 24h 节流，避免频繁请求 GitHub API

### 修复
- **诊断页亮度卡片**：新增"环境光传感器"检测行（第 3 行），汇总状态从"2/2就绪"改为"3/3就绪"
- **诊断页行内控件定位**：修复 SyncDiagWidth 二次覆盖 Label 宽度导致按钮/标签被遮挡的 bug
  - 标签宽度改为 MeasureString 动态计算（最小 80px），支持长文字不截断
  - 删除 SyncDiagWidth 中强制修改 Label 宽度的有害循环
  - 改用 row.Resize 事件统一管理标题/副标题宽度和标签/按钮位置
- **Label Anchor=Right 移除**：改为手动 resize 定位，避免初始 row.Width 不准导致的控件跨界

### 构建/打包
- Inno Setup 安装脚本版本号同步 1.0722.1
- bin 目录产物：`Traynexus-Setup-1.0722.1.exe` + `Traynexus.exe`
